...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        train_acc_sum = sum([torch.mean(x) for x in metrics["train_acc"]])
        logger.add_scalar("train_accuracy",  train_acc_sum / len(metrics["train_acc"]), global_step)
        ...
        val_acc_sum = sum([torch.mean(x) for x in metrics["val_acc"]])
        logger.add_scalar("val_accuracy", val_acc_sum / len(metrics["val_acc"]), global_step)
